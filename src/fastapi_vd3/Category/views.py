...
async def create_category(category_create: CategoryCreate, session: AsyncSession = Depends(get_async_session), admin_user: User = Depends(require_admin)):
...
async def delete_category(category_id: uuid.UUID, session: AsyncSession = Depends(get_async_session), admin_user: User = Depends(require_admin)):
    category = await service.get_category_by_id(category_id, session)
    if category is None:
        raise HTTPException(
            status_code=404, detail="Không tìm thấy danh mục với ID đã cho.")
...
async def update_category(category_id: uuid.UUID, category_update: CategoryCreate, session: AsyncSession = Depends(get_async_session), admin_user: User = Depends(require_admin)):
    category = await service.get_category_by_id(category_id, session)
    if category is None:
        raise HTTPException(
            status_code=404, detail="Không tìm thấy danh mục với ID đã cho.")
